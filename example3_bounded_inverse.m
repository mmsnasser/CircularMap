...
wehv = weh(abs(weh)>=0).';
%%
[xev,yev]=meshgrid([-0.995,-0.98,-0.965,-0.8,-0.62,-0.6,-0.57,-0.4,-0.2,0,0.14,0.17,0.2,0.4,0.6,0.8,0.85,0.88],-1:0.001:1);
wev=xev+i.*yev;
wevo = wev-cntd(1);
...
hold on
box on
plot(real(wehv),imag(wehv),'or','MarkerSize',1)
plot(real(wevv),imag(wevv),'ob','MarkerSize',1)
for k=1:m+1
...
hold on
box on
plot(real(zehv),imag(zehv),'or','MarkerSize',1)
plot(real(zevv),imag(zevv),'ob','MarkerSize',1)
for k=1:m+1
